...
const int TdsSensorPin = A3; // Pin analog yang terhubung ke sensor TDS
const float calibrationFactor = 300000.0; // Faktor kalibrasi untuk 300,000 ppm
GravityTDS gravityTds;
float nilaitds; // Menampung nilai Suhu pada function
float tdsValue; // Menampung Value suhu
...
char ssid[] = "saya"; // your network SSID (name)
char pass[] = "12345678";    // your network password
int status = WL_IDLE_STATUS; // the Wifi radio's status

char server[] = "192.168.4.81";
...
void loop()
{
  ...
  // TempValue = 31; // set testing data suhu
  ...
  Serial.print("Kadar Salinitas Air:\t\t");
...
//   HTTPClient http;
//   String apiUrl = "192.168.249.81/irigasi/post-esp-data.php";

//   // Mengatur tipe konten sebagai aplikasi/x-www-form-urlencoded
//   http.begin(apiUrl);
//   http.addHeader("Content-Type", "application/x-www-form-urlencoded");

//   // Membuat payload untuk dikirimkan ke server
//   String postData = "ph=" + String(pHValue) + "&keruh=" + String(TurbiValue) + "&suhu=" + String(TempValue);

//   // Melakukan request POST dan mengirimkan payload
...
void httpRequest(String data)
{
  ...
  if (client.connect(server, 80)) {
    ...
    String postData = "suhu=" + String(TempValue) + "&keruh=" + String(TurbiValue) + "&ph=" + String(pHValue) + "&tds=" + String(tdsValue);
    
    // Kirim HTTP POST request
    client.println("POST /tambak/post-esp-data.php HTTP/1.1");
    client.println("Host: 192.168.4.81");
...
{                                       
  // Mendapatkan nilai TDS
    float rawTdsValue = gravityTds.getTdsValue();
    // Mengalibrasi nilai TDS
    float calibratedTdsValue = rawTdsValue * calibrationFactor / 100000.0; // Sesuaikan dengan faktor kalibrasi
  return nilaitds;
}
